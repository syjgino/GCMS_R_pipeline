    ...
    while(~converge && its<max_it)
        ...
        if (fix_p)
            ...
            if D_1_restriction==0 % generates new p_2, p_1, p_0 by varying p_0 and p_1 
                ...
                % Dylan: now D_1_restriction=1, so varies p_2 and restricts p_1, p_0 accordingly
      ...
      for i= 1:(length(elong_unfix)-1)
       ...
       %%%%%%%%%%%%%%%%%%% End of Setting new p, s and e series for next iteration %%%%%%%%%%%%%%%%
        ...
        if((its<=100 && mod(its,20)==0) || mod(its,100)==0)    
            if  (numel(elong)>=3)
            display(['Iteration:',num2str(its),' Loglik=',num2str(log_lik), ' p2=',num2str(p(3)),' s=',num2str(elong(1)), ' e0=',num2str(elong(2)), ' e1=',num2str(elong(2)), ' e2=',num2str(elong(3))]);
            end 
        end
