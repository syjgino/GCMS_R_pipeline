...
#######USER INTERFACE ##########
cutoff_control= 10000## for quality control, if AUC<cutoff_control, it's treated as bad injection, [cutoff, because of atune in chol for lib 7/8]
InControl="190"
InControl.fix=F
...
for (inputfile in inputfiles){
  ...
  if (isFA) {
    ...
    for ( i in 1:length(lipids)){
      ...
      temp_ncol=min(ncol(FA_AUC),(FA_ID$right[i]-FA_ID$left[i]+1))
      FA_AUC[,1:temp_ncol] = AUCdata[,FA_ID$left[i]:(FA_ID$left[i]-1+temp_ncol)]  
