...
if (T) {
  ...
  sheet2ignore=c("standard")
  #sheet2ignore=c("contents","standard",
...
if ( is.null(Rda.files)){  
  print("please select your Summary...Rda file with to plot")
  Rda.files=choose.files()
}
setwd(dirname(Rda.files[1]))
basename(Rda.files)
#######################
...
for ( i.file in 1:length(Rda.files) ) {
  ...
  setwd(dirname(Rda.files[i.file]))
  file=basename(Rda.files[i.file])
  foldername=paste0("Figures for ",file)
  if (grepl("Rda",file)){
  ...
  for ( pa in names(quants.formal)){
    ...
    if (is.null(name2plot)){
      ...
      name2plot=which(grepl("SampleName",c))
    }
...
if (write2xlsx){
  print(paste0("Figures are in the folder: ",foldername))
  print(paste0("Mean and Sd are in file [",outname,"]"))
}
Rda.files=NULL

print("Congratulations! End of the pipeline!")
